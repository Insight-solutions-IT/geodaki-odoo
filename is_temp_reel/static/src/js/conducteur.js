...
export class FleetMapComponent3 extends Component {
    map;
     marker_list=[];
     marker_list2=[];
     vehiculesGlobal = [];
     ...
     setup() {
        ...
        onMounted(async ()=>{
            ...
            if (localStorage.getItem('firstVisit') != '3') {
...
$('head').append(`<link href="${window.location.origin}/is_temp_reel/static/css/conducteur.css"" rel="stylesheet" id="newcss_conducteur" />`);
    ...
    //         // });
            ...
            this.notification.add(
                this.env._t('Analyse par Conducteur'),
                {
                  type: 'success',
                }
              );
...
// } 
    ...
    async uncheckAllCheckboxes() {
        ...
        resetButton.addEventListener("click", async function () {
            ...
            self.firstCall=true;
            self.map.setZoom(10);
            self.map.setCenter({lat: 34.0556680, lng: -6.8138070});
...
if (rotation >= 360) {
    ...
    async treeJs2(){
        ...
        // <img class="img img-fluid" style="width:38px;height:38px;border-radius:20px;margin-right:10px" alt="Fichier binaire"
        //                               src="${window.location.origin}/web/image?model=hr.employee&id=${vehicle.id}&amp;field=image_128&amp;unique=1693390267000"
        //                                    name="image_128"
        //                                   style="width: 80px; height: 80px; object-fit: cover; margin-bottom: 5px;">

        var dataConducteur=[]
        conducteurs.forEach(vehicle=>{
                    dataConducteur.push({id:vehicle.id,text:`<div style="height:fit-content;display:flex;align-items:center"><img class="img img-fluid" style="width:38px;height:38px;border-radius:20px;margin-right:10px" alt="Fichier binaire"
                                           src="${window.location.origin}/web/image?model=hr.employee&id=${vehicle.id}&amp;field=image_128&amp;unique=1693390267000"
                                            name="image_128"
                                           style="width: 80px; height: 80px; object-fit: cover; margin-bottom: 5px;"><div style="display:flex;align-items:center;"><span class="vehicle-device" style="margin-right:10px;width:190px">  ${vehicle.name} </span><img class="vehicle-icon clickable3" id="${vehicle.id}" src="/fleet_monitoring/static/img/vehicules/navigate.png"></div></div>`,icon: 'none'})      
                })
...
$(document).on('click', '.clickable3', async (event) => {
    ...
    if(result.length>0){
        ...
        popup.classList=["popup-conducteur"];
        popup.id="popup"
        popup.innerText="Conducteur principale"
        this.notification.add(
            this.env._t('Conducteur principale'),
            {
              type: 'success',
            }
          );
...
//             );
        ...
        // document.getElementById("mapid").appendChild(popup);
        ...
        this.interval5 = window.setInterval(async function(){
            ...
            console.time('timer1')
            console.timeEnd('timer1')
            console.log("interval conducteur");
            ...
            if(result.length>0){
                ...
                // self.renderCircuit(v)
                self.get_map_data()
                
                console.timeEnd('timer2')}else{
                    clearInterval(self.interval5)
                    self.delete_old_marker();
                    self.delete_old_marker3();
                    alert("conducteur non planifié pour ce temp");
        ...
        if(result2.length>0){
            ...
            popup.classList=["popup-conducteur"];
            popup.id="popup"
            popup.innerText="Conducteur Secondaire"
            // this.notification.add(
            //     this.env._t('Conducteur Secondaire')
            //   );
            this.notification.add(
                this.env._t('Conducteur Secondaire'),
                {
    ...
    createNestedData2(vehicles, groups) {
        ...
        vehicles.forEach(vehicule =>{
            temp.push({id: 'c-'+vehicule.id, groupid: vehicule.groupid, name: vehicule.name })
        })
        vehicles=temp;
        var self=this;
        ...
        const buildVehicles = (groupId, parentGroupId) => {
            const groupVehicles = vehicles
                .filter(
                    (vehicle) =>
                        (vehicle.groupid[0] === groupId &&
                            vehicle.groupid[0] === parentGroupId) ||
                        vehicle.groupid[0] === parentGroupId
                )
                .map((vehicle) => {
...
FleetMapComponent3.template = "is_temp_reel.FleetMap3";
registry.category("actions").add("action_is_temp_reel4", FleetMapComponent3);
