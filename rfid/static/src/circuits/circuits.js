...
export class CircuitsMap extends Component {
  setup() {
    ...
    onMounted(async () => {
        ...
        if (localStorage.getItem('firstVisitRFID_PAR_CRCUIT') !== '1') {
          // This is the first visit, so set the flag
          localStorage.setItem('firstVisitRFID_PAR_CRCUIT', '1');
          location.reload();
        }
        ...
        setTimeout(()=>{
          localStorage.setItem('firstVisitRFID_PAR_CRCUIT', 'null');
      },3000);
        await this.initMap();
    });
  }
 async loadZones(){
  ...
  async loadAllGroups() { 
    ...
    document
      ...
      .addEventListener("click", async (e) => {
            ...
            content:
               `<table> 
                  ...
                  <tr>
                    ...
                    <td> ${element.freqcollecte} </td>
                  </tr> 
                  <tr> 
                    <td>Fréquence Lavage</td><td> ${element.freqlavage} </td>
                  </tr>
                  <tr> 
                    <td>Type</td>
                    <td> ${element.name} </td>
        ...
        // $("#DataDiv").append(` <div  id="myloader" style="
        //     display: block;
        //     z-index: 99;
        //     height: 22vw;
        //     width: 100%;
      ...
      document.getElementById('is_rfid_home').addEventListener('click', async (e)=>{
        ...
        try {
          ...
          await Promise.all([
            (result = await rpc.query({
              model: "is_rfid.readed_bacs_in_cir",
              method: "delete",
              args: [[], this.uid ],
            })),
          ]);
  ...
  async drawRFID(mapi, vehicules, circuit, start, end, etat, type) {
    // if(etat==='tous') etat  =''
    console.warn(start, end,vehicules, circuit, type, etat);
    type = type.replace("tous", "");
    if (type == "tous") type = "";
    console.warn(start, end, circuit, type, etat);
    let bacs = await this.loadBacs(type);
    let rfid = await this.loadRFIDForCircuit(start, end, vehicules,circuit, type, etat);
  ...
  async loadRFIDForCircuit(start, end, vehicules, circuitid, type, etat) {
    let bacs = [];
    try {
      await Promise.all([          
        (bacs = await rpc.query({  
          model: "is_rfid.vrfid",  
          method: "getRFIDCir",    
          args: [[], start, end,vehicules, circuitid, type, etat],
        })),
      ]);
    } catch (error) {
      console.warn(error);
  ...
  async displayData(mapi, vehicleId, start, end, etat, type) {
    ...
    const action = this.env.services.action;
      ...
      try {  
        ...
        document.getElementById('SelectedVehText').innerHTML = vehicule.map(v=>v.device);
        domain = `[
    ...
    if (action) {
      ...
      action
        .doAction({
          type: "ir.actions.act_window",
          name: "Open Vehicle Tree View",
          res_model: "is_rfid.readed_bacs_in_cir",
          domain: domain,
          views: [[false, "tree"]],
          view_mode: "tree",
          target: "new",
  ...
  createNestedData(vehicles, groups) {
    ...
    try {
            ...
            return {
              ...
              text: `
                                <div class="vehicle-node1">
                                    ...
                                    <div class="imgblock"> 
                                        <img class="vehicle-icon clickable_circuit" 
      ...
      $(document).on("click", ".clickable_circuit", async (event) => {
        document.getElementById('is_rfid_loader').style.display='flex'
        ...
        if(vehiculePlanifie.length==0){
          document.getElementById('is_rfid_loader').style.display='none'
          this.notification.add(
            this.env._t('Circuit non planifié', 'danger')
          );
          return;
        } 
        ...
        await this.drawRFID(this.map,vehicles, id,start,end,this.state.etat_conteneur,this.state.type_afichage);
        await this.displayData(this.map, vehicles, 'start', 'end', 'etat', 'type')
...
CircuitsMap.template = "rfid.circuitTemplate";
registry.category("actions").add("action_window_map_owl_circuits", CircuitsMap);
