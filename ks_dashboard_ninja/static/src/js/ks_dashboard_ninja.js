...
//            $('#ks_dashboard_title_input').val(this.ks_dashboard_data.ks_child_boards[this.ks_dashboard_data.ks_selected_board_id][0]);
        ...
        ksOnQuickEditView: function(e) {
         ...
         loadScriptchart: async function(src, integrity) {
        return new Promise((resolve, reject) => {
            const script = document.createElement('script');
            script.src = src;
        ...
        start: async function() {
            var self = this;
            // await self.loadscript('https://cdn.jsdelivr.net/npm/chart.js');
            // await self.loadscript('https://cdnjs.cloudflare.com/ajax/libs/chartjs-plugin-datalabels/2.2.0/chartjs-plugin-datalabels.min.js');
            // await self.loadscript('https://cdnjs.cloudflare.com/ajax/libs/chartjs-plugin-zoom/2.0.1/chartjs-plugin-zoom.min.js');

            // await self.loadScriptchart( `https://cdnjs.cloudflare.com/ajax/libs/chartjs-plugin-zoom/2.0.1/chartjs-plugin-zoom.min.js` , 
                    ...
                    options.tooltips.callbacks = {
                        label: function(tooltipItem, data) {
                            ...
                            if (ks_selection === 'monetary') {
                                ...
                                var ks_field = chart_data.ks_field;
                                // ks_type = field_utils.format.char(ks_field);
                                k_amount = field_utils.format.float(k_amount['y'], Float64Array, {digits:[0,item.ks_precision_digits]});
