...
repeatCount="indefinite" />
    ...
    onMounted(() => {
      ...
      document.getElementById("DisplayTree").style.marginLeft = "352px";
  ...
  async startStream() {
    try {
      ...
      const isStream = this._toggleStream();
      if (!isStream) {
        player.pause();
      }
      const data = {
      ...
      await this.socket.emit("toggleStream", data);
      console.log(isStream);
      streambtn.innerText = isStream ? "Stop Stream" : "Start Stream";

      //window.location.reload()
      var publisherName = this.device.trim();
      this.updateVideoSource(publisherName);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  updateVideoSource(newSource) {
    try {
      ...
      setTimeout(() => {
        ...
        sourceElement.src = `http://newrabat.geodaki.com:5001/hls/${newSource}.m3u8`;
        player.src({
          type: "application/vnd.apple.mpegurl",
          src: `http://newrabat.geodaki.com:5001/hls/${newSource}.m3u8`,
        });
  ...
  Load() {
    try {
      let player = videojs("my-video");
      var sourceElement = document.getElementById("video-source");
      sourceElement.src = `http://newrabat.geodaki.com:5001/hls/${this.device}.m3u8`;
      player.src({
        type: "application/vnd.apple.mpegurl",
        src: `http://newrabat.geodaki.com:5001/hls/${this.device}.m3u8`,
      });
      player.load();
      //player.play();
      console.log(`http://newrabat.geodaki.com:5001/hls/${this.device}.m3u8`);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  createNestedData(vehicles, groups) {
    ...
    try {
      ...
      $(document).on("click", ".clickable", async event => {
        ...
        fetch("http://newrabat.geodaki.com:5002/api/status", requestOptions)
          .then(response => response.json())
          .then(result => {
            console.log(result);
            let isStream = result.stream;
            streambtn.innerText = isStream ? "Stop Stream" : "Start Stream";

            streambtn.style.backgroundColor = isStream ? "#6e4aa7" : "green";
          })
...
StreamVideo.template = "is_acces_video.StreamVideoTemp";
registry.category("actions").add("is_acces_video.StreamVideo", StreamVideo);
